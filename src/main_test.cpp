...
bool CBlock::AcceptBlock()
{
    ...
    if (GetBlockTime() <= pindexPrev->GetPastTimeLimit() || GetBlockTime() + GetClockDrift(GetBlockTime(), nHeight) < pindexPrev->GetBlockTime())
      ...
      if(nHeight > 1457458 ) {
		if(IsProofOfWork() && GetBlockTime() <= (pindexPrev->GetBlockTime() + 30) ) {    //MBC 20190115
			return error("AcceptBlock() : block's timestamp is too early : block time (%d), Pre Block time + 30 sec (%d) ", GetBlockTime(), pindexPrev->GetBlockTime() + 40);
		}
     } 
