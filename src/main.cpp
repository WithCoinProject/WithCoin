                        ...
                        bool* pfMissingInputs)
{
    ...
    uint256 hash = tx.GetHash();
    {
        LOCK(cs);
        if (mapTx.count(hash)) {
			// printf("MBC CTxMemPool::accept  accepted mapTx.count false \n" );
			return false;
		}
    }
    if (fCheckInputs)
        if (txdb.ContainsTx(hash)) {
			//printf("MBC CTxMemPool::accept  txdb.ContainsTx  false \n" );
            return false;
		}
    ...
    if (fCheckInputs)
    {
		...
		//printf("MBC CTxMemPool::accept  FetchInputs start \n" );
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, unsigned int nTime, int64_t nFees, int64_t nValueIn, uint256 prevHash)
{
	...
	if (pindexBest->nHeight+1 > nTargetBlockHalve) {

		int64_t nRewaMCCoinYear = MAX_MINT_PROOF_OF_STAKE_NEW;
		int64_t nSubsidy = nCoinAge * nRewaMCCoinYear / 365 / COIN;
...
//
unsigned int ComputeMinWork(unsigned int nBase, int64_t nTime, int nHeight)
{
    //return ComputeMaxBits(bnProofOfWorkLimit, nBase, nTime);

	CBigNum bnTargetLimit = bnProofOfWorkLimit_new(nHeight);   //20180905
	return ComputeMaxBits(bnTargetLimit, nBase, nTime);   //20180905
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	CBigNum bnTargetLimit = bnProofOfWorkLimit;   //20180905
	...
	int cur_height = pindexPrev->nHeight + 1; 
	if (!fProofOfStake)
    {		
		bnTargetLimit = bnProofOfWorkLimit_new(cur_height);		   //20180905 		
	}
	//printf(">> cur_height = %d  \n" , cur_height); 
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, int nHeight)
{
	...
	if (bnTarget <= 0 || bnTarget > bnTargetLimit)          //20180905
        return error("CheckProofOfWork() : nBits below minimum work");
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
	...
	//printf("MBC CTransaction::ConnectInputs start \n");
    ...
    if (!IsCoinBase())
    {
		...
		//printf("MBC CTransaction::ConnectInputs vin.size [%d] \n", vin.size());
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
			...
			//printf("MBC CTransaction::ConnectInputs prevout.n [%d] txPrev.vout.size = [%d] txindex.vSpent.size = [%d] \n", prevout.n,  txPrev.vout.size(),txindex.vSpent.size() );
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
			...
			//printf("MBC CTransaction::ConnectInputs fBlock=[%d] nBestHeight = [%d] Checkpoints::GetTotalBlocksEstimate = [%d] \n", fBlock,  nBestHeight, Checkpoints::GetTotalBlocksEstimate() );
            ...
            if (!(fBlock && (nBestHeight < Checkpoints::GetTotalBlocksEstimate())))
            {
				...
				//printf("MBC CTransaction::VerifySignature start \n");
                if (!VerifySignature(txPrev, *this, i, 0))
                {
                    //MBC return DoS(100,error("ConnectInputs() : %s VerifySignature failed", GetHash().ToString().substr(0,10).c_str()));
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
	...
	if (fCheckPOW && IsProofOfWork() && !CheckProofOfWork(GetHash(), nBits, nBestHeight + 1))   //20180905
        return DoS(50, error("CheckBlock() : proof of work failed"));

    // Check timestamp
    if (GetBlockTime() > GetAdjustedTime() + GetClockDrift(GetBlockTime(), nBestHeight + 1))
    ...
    if (GetBlockTime() > (int64_t)vtx[0].nTime + GetClockDrift((int64_t)vtx[0].nTime,  nBestHeight + 1))
    ...
    //         int64_t nNow = GetAdjustedTime(); // Huns 2019.03.21 - about MBC Get current time funtion.
			    if((nNow > 1556636400 ) && ((nBestHeight + 1) > 1625800)) {   // Huns 2019.03.21 - about2019.04.01 00:00  with4.7 security rule start.(Generate block every 50block POS)
			if(((nBestHeight + 1) % 50) == 0 ) {    //MBC 20190115  			

				return error("CheckBlock() : Pow Create Error : block height (%d) ", nBestHeight + 1);
...
////////  2019 MBC check block /////////////////////////////////////////////////////////////////////
bool CBlock::CheckBlock_Load(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    // These are checks that are independent of context
    // that can be verified before saving an orphan block.
...
bool CBlock::AcceptBlock()
{
    ...
    if (GetBlockTime() <= pindexPrev->GetPastTimeLimit() || GetBlockTime() + GetClockDrift(GetBlockTime(), nHeight) < pindexPrev->GetBlockTime())
        return error("AcceptBlock() : block's timestamp is too early");

        if(nNow > 1556636400  && nHeight > 1578350){   // Huns 2019.03.21 - about2019.04.01 00:00  with4.7 security rule start.(Time between POW block and POW block is 30sec over.)
		if(IsProofOfWork() && GetBlockTime() <= (pindexPrev->GetBlockTime() + 32) ) {    //MBC 20190115
			return error("AcceptBlock() : block's timestamp is too early : block time (%d), Pre Block time + 30 sec (%d) ", GetBlockTime(), pindexPrev->GetBlockTime() + 32);
		}
     } 
	  if(nNow > 1556636400  && nHeight > 1625800){  // Huns 2019.05.08
		if(IsProofOfWork() && (nHeight % 50) == 0 ) {    //MBC 20190115           
           return error("AcceptBlock() : Pow Create Error : block height (%d) ", nHeight);
               }
	  }
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain && !Checkpoints::WantedByPendingSyncCheckpoint(hash))
    {
        ...
        if (pblock->IsProofOfStake())
            ...
            bnRequired.SetCompact(ComputeMinWork(GetLastBlockIndex(pcheckpoint, false)->nBits, deltaTime, nBestHeight + 1));
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    if (nSearchTime > nLastCoinStakeSearchTime)
    {
        ...
        {
            if (txCoinStake.nTime >= max(pindexBest->GetPastTimeLimit()+1, pindexBest->GetBlockTime() - GetClockDrift(pindexBest->GetBlockTime(), pindexBest->nHeight)))
            {
			    // make sure coinstake would meet timestamp protocol
                //    as it would be the same as the block timestamp
                vtx[0].nTime = nTime = txCoinStake.nTime;
                nTime = max(pindexBest->GetPastTimeLimit()+1, GetMaxTransactionTime());
                nTime = max(GetBlockTime(), pindexBest->GetBlockTime() - GetClockDrift(pindexBest->GetBlockTime(), pindexBest->nHeight));
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
      ...
      //  if (pfrom->nBlocksRequested * 80 > pfrom->nSendBytes) {
		//	printf("pfrom->nBlocksRequested [%d]>=[%d] \n",pfrom->nBlocksRequested * 80, pfrom->nSendBytes);
	//		break;
	//	}
                        ...
                        if (pfrom->pfilter)
                        {
                            ...
                            BOOST_FOREACH(PairType& pair, merkleBlock.vMatchedTxn)
                                if (!pfrom->setInventoryKnown.count(CInv(MSG_TX, pair.second))) {
									//printf("mapBlockIndex PairType PushMessage \n");
									//block.vtx[pair.first].print();
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
			...
			//printf("  partner %s, got inventory: %s  %s\n", pfrom->addr.ToString().c_str(), inv.ToString().c_str(), fAlreadyHave ? "have" : "new");
    ...
    else if (strCommand == "getheaders")
    {
        ...
        int nLimit = 1500;
        //printf("partner %s, getheaders %d to %s\n", pfrom->addr.ToString().c_str(), (pindex ? pindex->nHeight : -1), hashStop.ToString().substr(0,20).c_str());
        ...
        if (tx.AcceptToMemoryPool(txdb, true, &fMissingInputs))
        {
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                     ...
                     ++mi)
                {
                    ...
                    {
                        //printf(" partner %s,  accepted orphan tx %s\n", pfrom->addr.ToString().c_str(), orphanTxHash.ToString().substr(0,10).c_str());
                        SyncWithWallets(tx, NULL, true);
                    ...
                    else if (!fMissingInputs2)
                    {
                        // invalid orphan
                        vEraseQueue.push_back(orphanTxHash);
                        //printf(" partner %s,  removed invalid orphan tx %s\n", pfrom->addr.ToString().c_str(), orphanTxHash.ToString().substr(0,10).c_str());
                    }
                }
            }
    ...
    while (true)
    {
        ...
        if (vRecv.end() - pstart < nHeaderSize)
        {
            if ((int)vRecv.size() > nHeaderSize)
            {
                ...
                //printf("\n\nPROCESSMESSAGE MESSAGESTART NOT FOUND vRecv.size = [] nHeaderSize= []  \n\n", vRecv.size(), nHeaderSize);
